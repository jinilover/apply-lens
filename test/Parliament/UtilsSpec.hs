...
adjustListCheck =
    describe "UtilsSpec adjustListCheck" $
      it "The ints are adjusted and always sum up to the required value" $
        forAll genInt $ \newbase ->
          forAll genInts $ \xs ->
            let xs' = adjustList newbase id xs in
            xs == [] && xs == xs' ||
